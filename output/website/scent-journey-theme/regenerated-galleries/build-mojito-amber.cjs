...
function text(lines,x,y,size=60,leading=1.22,weight=400,color='#231c1a',spacing=0) {
  return `<text x="${x}" y="${y}" font-family="${weight >= 500 ? 'Cenzo Flare Bold' : 'Helvetica Now Text Regular'}" font-size="${size}" font-weight="${weight}" letter-spacing="${spacing}" fill="${color}">${lines.map((s,i)=>`<tspan x="${x}" dy="${i ? size*leading : 0}">${esc(s)}</tspan>`).join('')}</text>`;
}
...
async function run() {
  ...
  for(const s of scents) {
    ...
    for(let frame=1;frame<=12;frame++) {
      ...
      const svgLayer=overlay(frame,s);
      let layer=null;
      if(svgLayer) {
        const rendered=spawnSync('python3',[path.join(base,'render-mojito-amber-type.py')],{input:svgLayer,maxBuffer:20*1024*1024});
        if(rendered.status!==0) throw new Error(rendered.stderr.toString());
        layer=rendered.stdout;
      }
      ...
      records.push({handle:s.handle,frame:number,source:src,sourceDimensions:[original.width,original.height],master:out,masterDimensions:[2048,2048],webDimensions:[1200,1200],type:layer?'Deterministic supplied-font typography over unchanged clean ImageGen base':'New ImageGen photograph',font:layer?'Cenzo Flare Bold (headings); Helvetica Now Text Regular (body)':''});
