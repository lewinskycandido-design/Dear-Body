...
REG=str(ROOT.parent/'source-assets/fonts/HelveticaNowText-Regular/HelveticaNowText-Regular.ttf')
BOLD=str(ROOT.parent/'source-assets/fonts/CenzoFlare-Bold/CenzoFlare-Bold.ttf')
...
def text(im,value,x,y,width,size=56,bold=False,color=BURGUNDY,leading=None,max_bottom=1950):
 ...
 for line in lines:
  ...
  if bounds[2]>x+width+1 or bounds[3]>max_bottom:raise ValueError(('Text exceeds frame',line,bounds,width,max_bottom))
  d.text((x,y),line,font=f,fill=color,stroke_width=0);y+=leading
 layout_audit.append({'text':value,'font_family':'Cenzo Flare Bold' if bold else 'Helvetica Now Text Regular','font_size':size,'line_count':len(lines),'ending_y':y})
 return y
...
for handle,d in DATA.items():
 ...
 text(im,'50 ml',93,622,860,122,False)
 ...
 text(im,'50 ml',69,1120,590,100,False,color=CREAM)
 text(im,'1.69 fl. oz.',73,1246,590,45,color=CREAM)
 text(im,'PRODUCT CODE',73,1400,600,29,True,color=CREAM)
 text(im,d['code'],70,1448,590,58,False,color=CREAM)
 text(im,'BARCODE',73,1570,590,29,True,color=CREAM)
 text(im,d['barcode'],70,1620,590,49,False,color=CREAM)
 x=822
 ...
 (ROOT/handle/'EXPORT-NOTES.md').write_text('''# Export notes\n\nAll photographic sources were newly generated for this request using the built-in ImageGen tool. No legacy gallery images were reused. Native ImageGen returned 1254×1254 despite the 2048 request and exposes no size argument; original native sources are preserved in `working/`. Final photography is transparently resampled with Lanczos to the required 2048×2048 sRGB PNG. All deterministic type is rendered directly at 2048×2048.\n\nThe 2026-09-24 typography refresh uses the actual user-supplied Cenzo Flare Bold TTF for promotional headings and Helvetica Now Text Regular TTF for body copy. The user explicitly supplied Text Regular in place of the earlier Display specification. These are embedded raster text from the supplied binaries, with no system-font substitution. Physical bottle/canister label typography is unchanged. Earlier system-font versions are safely archived under `archive/2026-09-24-system-font-version/`.\n\nOnly frames 04/08/09/10/11/12 were re-rendered from clean source photos and deterministic layout code. All photos and verified copy are preserved; no image patching or ImageGen edit was required because the promotional captions were separate deterministic overlays. Optimized website derivatives are 1200×1200 progressive JPEG quality 88.\n''')
(ROOT/'mistened-charme-layout-audit.json').write_text(json.dumps(layout_audit,indent=2,ensure_ascii=False)+'\n')
print(f'Exported {2*(len(SELECTED) if SELECTED else 12)} 2048 PNG frames and matching 1200 JPEG derivatives; supplied-font typography bounds passed.')
