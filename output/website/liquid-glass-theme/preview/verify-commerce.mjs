...
const rowSource = await fs.readFile(path.join(theme, 'snippets/db-fragrance-row.liquid'), 'utf8');
for (const fixture of [
  { name: 'Editorial row excludes zero price and uses real paid price', variants: [{ price: 0, available: true }, { price: 75000, available: true }], price: '£750.00 GBP', action: 'Choose options' },
  { name: 'Editorial row uses From only for different positive prices', variants: [{ price: 95000, available: true }, { price: 0, available: true }, { price: 75000, available: true }], price: 'From £750.00 GBP', action: 'Choose options' },
  { name: 'Editorial unpriced product remains available soon', variants: [{ price: 0, available: true }], price: 'Available soon', action: 'View fragrance' },
  { name: 'Editorial unavailable paid product displays sold out', variants: [{ price: 75000, available: false }], price: '£750.00 GBPSold out', action: 'View fragrance' },
  { name: 'Editorial missing product URL emits no purchase or detail links', variants: [{ price: 75000, available: true }], price: '£750.00 GBP', url: '' },
]) {
  const markup = await liquid.parseAndRender(rowSource, { product: { id: 77, title: 'Mojito Metallique', url: fixture.url ?? '/products/merchant-product', variants: fixture.variants, featured_image: { src: '/merchant-product.jpg' } }, collection_title: 'For Her', index: 1, section_id: 'test' });
  const rowDom = new JSDOM(markup); const rowDocument = rowDom.window.document;
  verify(fixture.name, () => {
    assert.equal(rowDocument.querySelector('.db-fragrance-row__price').textContent.trim(), fixture.price);
    assert.equal(rowDocument.querySelector('.db-fragrance-row__description').textContent.trim(), 'Soft, creamy sweetness with a light fruity glow.');
    assert.equal(rowDocument.querySelector('.db-fragrance-row__media img').getAttribute('src'), '/merchant-product.jpg');
    assert.equal(rowDocument.querySelector('form'), null);
    if (fixture.url === '') assert.equal(rowDocument.querySelectorAll('a').length, 0);
    else { assert.equal(rowDocument.querySelector('.db-fragrance-row__actions a').textContent.replace('↗', '').trim(), fixture.action); for (const a of rowDocument.querySelectorAll('a')) assert.equal(a.getAttribute('href'), '/products/merchant-product'); }
  }); rowDom.window.close();
}
