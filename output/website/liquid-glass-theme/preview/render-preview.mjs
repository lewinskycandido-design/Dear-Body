...
const previewPort = Number(process.env.PORT || 4174);
...
async function verifyLifestyleHomepage() {
  ...
  for (const category of categoryCollections) {
    ...
    if (!link || categoryDocument.querySelectorAll('[data-fragrance-row]').length !== category.products.length) throw new Error(`Lifestyle homepage lost access to the ${category.title} catalog.`);
    for (const product of category.products) {
      if (!categoryDocument.querySelector(`[data-fragrance-row] a[href="${product.url}"]`)) throw new Error(`${category.title} lost the ${product.title} detail-page link.`);
    }
  }
  checks.push('For Her and For Him navigation still reaches all six fragrance detail pages through their category catalogs');
  const mainNav = document.querySelector('.db-desktop-nav');
  const navigationLabels = [...mainNav.querySelectorAll('a,button')].map(link => link.textContent.trim());
  if (JSON.stringify(navigationLabels) !== JSON.stringify(['Home', 'Scent Finder', 'For Her', 'For Him', 'Our Story', 'Contact'])) throw new Error('DearBody navigation must retain the requested six destinations.');
  if (!mainNav.querySelector('button[data-scent-quiz-open]') || !document.querySelector('.db-story-seal .db-secondary-logo-art') || !document.querySelector('.db-footer-seal .db-secondary-logo-art')) throw new Error('Scent Finder or secondary brand marks are missing.');
  for (const category of categoryCollections) {
    const categoryDocument = new JSDOM(await fs.readFile(path.join(publicRoot, category.url, 'index.html'), 'utf8')).window.document;
    if (categoryDocument.querySelector('.db-product-grid')) throw new Error('For Her and For Him must use editorial rows, not a product grid.');
    const rows = [...categoryDocument.querySelectorAll('[data-fragrance-row]')];
    for (const row of rows) {
      if (!row.querySelector('.db-fragrance-row__copy h2') || !row.querySelector('.db-fragrance-row__media img') || !row.querySelector('.db-fragrance-row__description')?.textContent.trim()) throw new Error('Every editorial row needs a product heading, image and verified description.');
      if (row.querySelector('form') || !row.querySelector('.db-fragrance-row__price')?.textContent.includes('₱799.00 PHP') || !row.querySelector('.db-fragrance-row__price')?.textContent.includes('Sold out')) throw new Error('Approved-price editorial products must retain unavailable inventory behavior.');
    }
  }
  checks.push('Editorial collections render complete product rows; primary and secondary marks and six-item Scent Finder navigation are present');
  if (!document.querySelector('.db-lifestyle-scene--bag a[href="/collections/womens-perfume"]') || !document.querySelector('.db-lifestyle-scene--vanity a[href="/collections/mens-perfume"]') || !document.querySelector('.db-lifestyle-display [data-scent-quiz-open]')) throw new Error('Lifestyle sections must link both categories and offer the scent quiz.');
  checks.push('Homepage includes direct For Her and For Him lifestyle links plus a scent-finder callout');
...
async function build() {
  ...
  const report = {
    generatedAt: new Date().toISOString(), edition: 'DearBody Liquid Glass', previewPort, themeRoot, routes, validation,
