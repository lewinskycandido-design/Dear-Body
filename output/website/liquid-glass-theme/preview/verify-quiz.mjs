...
function createQuiz(mutateCatalog) {
  ...
  const trigger = document.querySelector('.db-hero [data-scent-quiz-open]');
...
await verify('Navigation Scent Finder opens the same quiz and mobile dismissal restores visible summary focus', () => usingQuiz(q => {
  const desktop = q.document.querySelector('.db-desktop-nav [data-scent-quiz-open]');
  assert.ok(desktop);
  q.click(desktop);
  assert.equal(q.dialog.open, true);
  q.click(q.dialog.querySelector('[data-quiz-close]'));
  assert.equal(q.document.activeElement, desktop);
  const menu = q.document.querySelector('.db-mobile-menu');
  menu.open = true;
  q.click(menu.querySelector('[data-scent-quiz-open]'));
  assert.equal(q.dialog.open, true);
  menu.open = false;
  q.click(q.dialog.querySelector('[data-quiz-close]'));
  assert.equal(q.document.activeElement, menu.querySelector('summary'));
}));
