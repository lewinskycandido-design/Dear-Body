...
The homepage **Find your scent** link opens a three-question modal in the Bauhaus theme. It asks which collection, scent direction and character the visitor prefers. Recommendations include the bundled product image, the exact approved scent description, a reason based on matched preferences, and an actual Shopify product-page link when available.

The quiz uses the existing brand colors with square controls and solid Bauhaus panels. Back, retake, close, keyboard focus cycling and Escape are supported. The result area scrolls on small screens while the close button stays visible. Answers stay in memory for the open quiz; no email address, tracking endpoint, storage or external quiz service is used. Closing and reopening starts again. Without JavaScript or native dialog support, the hero link opens the fragrance catalog.

## Shopify setup

Install `DearBody-Bauhaus-Shopify-Theme.zip` as a separate draft. The quiz is already included. A manual installation must preserve the full theme; the shared layout loads the quiz styles and script as well as the Bauhaus presentation stylesheet. Its main files are:

- `assets/dearbody-quiz.js`
- `assets/dearbody-quiz.css`
- `assets/dearbody-bauhaus.css`
...
These automated checks were rerun for the Bauhaus edition. Native browser dialog focus, Escape behavior, scrolling and visual contrast require a separate browser review; the automated modal shim cannot establish those details. Current sampled browser results are recorded in `VALIDATION.md`. Earlier Liquid Glass layout measurements are not Bauhaus results. This is local-preview validation, not a Shopify upload or publication.
