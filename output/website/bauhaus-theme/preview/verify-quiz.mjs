...
await verify('Mobile menu quiz closes back to the visible Menu summary', () => usingQuiz(q => {
  const menu = q.document.querySelector('.db-mobile-menu');
  const mobileTrigger = menu.querySelector('[data-scent-quiz-open]');
  menu.open = true;
  mobileTrigger.focus();
  menu.open = false;
  q.click(mobileTrigger);
