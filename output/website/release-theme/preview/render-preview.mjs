...
const previewPort = Number(process.env.PORT || 4197);
...
// Local layout fixtures only: no price or gender classification is asserted.
const priorityPrice = 0;
const products = catalog.map(([title, handle], index) => {
  ...
  return {
    id: index + 1, title, handle, url: `/products/${handle}`, type: '', price: priorityPrice, tags: [],
...
const collection = { id:100, sort_options: [{value:'manual',name:'Featured'},{value:'price-ascending',name:'Price: low to high'},{value:'price-descending',name:'Price: high to low'},{value:'title-ascending',name:'Alphabetically, A–Z'}], default_sort_by: 'manual', title: 'All fragrances', handle: 'all', description: '', products, products_count: products.length, all_products_count: products.length, url: '/collections/all' };
// Saved local source records define membership; no product gender is inferred.
const catalogCollections = JSON.parse(await fs.readFile(path.join(previewRoot, 'catalog-collections.json'), 'utf8'));
const categoryCollections = catalogCollections.collections.map(({id, title, handle, products: members}) => {
  const categoryProducts = members.map(handle => allProducts[handle]);
  if (categoryProducts.some(product => !product)) throw new Error(`Missing preview reference in ${handle}`);
  return { id, sort_options: collection.sort_options, default_sort_by: 'manual', title, handle, url: `/collections/${handle}`, description: '', products: categoryProducts, products_count: categoryProducts.length, all_products_count: categoryProducts.length };
});
...
const baseContext = {
  shop: { url: 'http://127.0.0.1:4197', name: 'DearBody Philippines', currency: 'PHP', policies: [], shipping_policy: null, refund_policy: null, privacy_policy: null, customer_accounts_enabled: false },
  routes: { root_url: '/', all_products_collection_url: '/collections/all', cart_url: '/cart', cart_add_url: '/cart/add', search_url: '/search', predictive_search_url: '/search/suggest', product_recommendations_url: '/recommendations/products', account_url: '/account', cart_change_url: '/cart/change', cart_update_url: '/cart/update' },
  request: { locale: { iso_code: 'en' }, design_mode: false, page_type: 'index' },
...
const previewBanner = `<aside class="db-preview-notice" aria-label="Local preview notice"><strong>DESIGN PREVIEW</strong><span>Official logo & webfonts pending · Local reference catalog; prices and inventory unconfigured · No orders are sent</span><nav aria-label="Preview pages"><a href="/">Home</a><a href="/pages/scent-finder">Scent Finder</a><a href="/products/mojito-metallique">Product</a><a href="/cart">Bag</a></nav></aside>`;
...
async function renderPage(templateName, route, extraContext = {}) {
  const context = { ...baseContext, request: { ...baseContext.request, path: route, page_type: templateName.split('.')[0] }, canonical_url: `http://127.0.0.1:${previewPort}${route}`, page_title: 'Dear Body Philippines', template: { name: templateName.split('.')[0], suffix: templateName.split('.')[1] || '' }, ...extraContext };
  ...
  // Neutral price text is preview-only and is never included in the upload theme.
  result = result.replace(/₱0\.00(?: PHP)?/g, 'PRICE SET IN SHOPIFY');
...
async function verifyMasterPages(routes) {
 ...
 checks.push('Six named reference launch cards; exact four-link navigation; master hero copy');
 for (const category of categoryCollections) {
  const categoryDoc = new JSDOM(await fs.readFile(path.join(publicRoot,category.url.slice(1),'index.html'),'utf8')).window.document;
  const expected = category.products.map(product=>product.title);
  const actual = [...categoryDoc.querySelectorAll('.db-discovery-products .db-card h3')].map(heading=>heading.textContent.trim());
  if (JSON.stringify(actual)!==JSON.stringify(expected)) throw new Error(`${category.url}: incorrect reference product membership`);
  const isWomen = category.handle === 'womens-perfume';
  if(!categoryDoc.querySelector('h1').textContent.includes(isWomen?'EVERY VERSION OF YOU':'PART OF YOUR STORY')) throw new Error(`${category.url}: incorrect hero`);
  for (const selector of ['.db-desktop-nav','.db-mobile-menu nav','.db-footer nav']) {
   const label=isWomen?'Women':'Men';
   const link=[...doc.querySelectorAll(`${selector} a`)].find(a=>a.textContent.trim()===label);
   if(link?.getAttribute('href')!==category.url) throw new Error(`${selector}: ${label} destination wrong`);
  }
  const heroLink=[...doc.querySelectorAll('.db-hero-links a')].find(a=>a.textContent.trim().toLowerCase().startsWith(isWomen?'shop women':'shop men'));
  if(heroLink?.getAttribute('href')!==category.url) throw new Error(`${category.url}: homepage button destination wrong`);
  if(!categoryDoc.querySelector('.db-desktop-nav [aria-current="page"]')) throw new Error(`${category.url}: active navigation missing`);
  checks.push(`${category.url}: three source-verified products, distinct hero, desktop/mobile/footer/hero destinations and active navigation`);
 }
 const allDoc=new JSDOM(await fs.readFile(path.join(publicRoot,'collections/all/index.html'),'utf8')).window.document;
 if(allDoc.querySelector('h1').textContent!=='All fragrances') throw new Error('All fragrances must not inherit a gender-specific hero');
 checks.push('All fragrances retains its independent collection identity');
...
async function build() {
  ...
  for (const category of categoryCollections) routes.push(await renderPage(category.handle === 'womens-perfume' ? 'collection.women' : 'collection.men', category.url, { collection: category, page_title: category.title }));
  ...
  const report = {
    ...
    genericPageChecks: ['Generic page template uses page content and page title without handle assumptions'],
    pageBannerChecks,
    lifestyleHomepageChecks,
    gaps: ['Local reference catalog is not a live Shopify read. Women/Men memberships and collection handles follow saved local source records; no current prices or availability are asserted.', 'Shopify-only form, section/group, and single-page pagination behavior is emulated around the actual Liquid markup.', 'Image CDN resizing/srcset and Shopify media players are not emulated.', 'No checkout, payment, email, customer session, inventory or theme-editor behavior is tested.', 'Official parser validates syntax, not complete Shopify theme-store requirements or runtime semantics.'],
  };
