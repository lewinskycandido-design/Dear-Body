...
await test('All discovery sections parse with Shopify Liquid parser and valid schemas', async () => {
  ...
  for (const name of ['db-scent-quiz', 'db-scent-finder', 'db-main-scent-finder', 'db-mood-discovery', 'db-main-collection', 'db-main-story']) {
...
await test('Mood cards support direct collection selection and unlinked editorial moods', async () => {
  const context = { section: { id: 'moods', settings: { enabled: true }, blocks: [] }, request: { design_mode: false } };
  context.section.blocks = [{ settings: { title: 'Confirmed feeling', description: 'Merchant-supplied mood.' } }];
  const editorial = await engine.parseAndRender(body(moodSource), context);
  assert.match(editorial, /<article class="db-mood-card"/);
  assert.ok(!editorial.includes('<a '));
  assert.ok(!editorial.includes('Shop by feeling setup'));
  context.section.blocks[0].settings.collection = { url: '/collections/confirmed' };
  ...
  context.section.settings.enabled = false;
  assert.equal((await engine.parseAndRender(body(moodSource), context)).trim(), '');
});
await test('Configured empty state and browse collection survive JavaScript enhancement', async () => {
  const settings = { fallback_heading: 'New profiles soon.', fallback_text: 'Read our supplied fragrance profiles.', browse_label: 'Browse the edit', browse_collection: { url: '/collections/merchant-edit' } };
  const html = await render(settings);
  assert.match(html, /New profiles soon/);
  assert.match(html, /href="\/collections\/merchant-edit"/);
  const dom = mount(html);
  assert.equal(dom.window.document.querySelector('.db-quiz-empty-title').textContent, settings.fallback_heading);
  assert.equal(dom.window.document.querySelector('[data-quiz-content] a').textContent, settings.browse_label);
  assert.equal(dom.window.document.querySelector('[data-quiz-content] a').pathname, settings.browse_collection.url);
  dom.window.close();
});
await test('Finder CTA uses selected pages and native routes without assumed handles', async () => {
  const content = await fs.readFile(path.join(theme, 'sections/db-scent-finder.liquid'), 'utf8');
  const context = { section: { id: 'cta', settings: { enabled: true, button_label: 'Start', browse_label: 'Browse' } }, settings: {}, routes: { all_products_collection_url: '/collections/all' } };
  assert.match(await engine.parseAndRender(body(content), context), /href="\/collections\/all"/);
  context.settings.scent_finder_page = { url: '/pages/my-fragrance-finder' };
  assert.match(await engine.parseAndRender(body(content), context), /href="\/pages\/my-fragrance-finder"/);
  context.section.settings.finder_page = { url: '/pages/my-other-quiz' };
  assert.match(await engine.parseAndRender(body(content), context), /href="\/pages\/my-other-quiz"/);
  context.section.settings.enabled = false;
  assert.equal((await engine.parseAndRender(body(content), context)).trim(), '');
});
await test('SEO follows explicit templates or collection IDs and preserves merchant metadata', async () => {
  const content = await fs.readFile(path.join(theme, 'snippets/db-seo.liquid'), 'utf8');
  const context = { field: 'title', template: { name: 'collection', suffix: '' }, collection: { id: 77, title: 'Merchant collection', url: '/collections/merchant-choice' }, settings: {}, page_title: 'Merchant collection', routes: { all_products_collection_url: '/collections/all' } };
  assert.equal((await engine.parseAndRender(content, context)).trim(), 'Merchant collection');
  context.settings.for_her_collection = { id: 77 };
  assert.match(await engine.parseAndRender(content, context), /Women's Perfume Philippines/);
  context.page_title = 'Merchant search title';
  assert.equal((await engine.parseAndRender(content, context)).trim(), 'Merchant search title');
  context.field = 'description'; context.page_description = 'Merchant search description';
  assert.equal((await engine.parseAndRender(content, context)).trim(), 'Merchant search description');
  context.page_description = ''; context.field = 'title'; context.page_title = 'Merchant collection'; context.template.suffix = 'men';
  assert.match(await engine.parseAndRender(content, context), /Men's Perfume Philippines/);
  context.template.suffix = 'launch';
  assert.match(await engine.parseAndRender(content, context), /Six Signature Fragrances/);
  Object.assign(context, { template: { name: 'page', suffix: 'scent-finder' }, page: { id: 4, title: 'My page', handle: 'anything-i-like' }, page_title: 'My page', settings: {} });
  assert.match(await engine.parseAndRender(content, context), /DearBody Scent Finder/);
});
