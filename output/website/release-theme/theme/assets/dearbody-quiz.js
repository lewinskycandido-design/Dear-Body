...
(() => {
  ...
  function browseLink(state) {
    return link(text(state.catalog.browseLabel) || 'Explore all fragrances', state.catalog.allProductsUrl, 'db-text-link');
  }
  function renderEmpty(root, state) {
    const content = root.querySelector('[data-quiz-content]');
    content.replaceChildren();
    content.append(node('h3', 'db-quiz-empty-title', text(state.catalog.fallbackHeading) || 'Your scent journey starts with a little curiosity.'));
    content.append(node('p', 'db-quiz-hint', text(state.catalog.fallbackText) || 'Our scent finder is being prepared. Explore the collection and discover the scent descriptions in the meantime.'));
