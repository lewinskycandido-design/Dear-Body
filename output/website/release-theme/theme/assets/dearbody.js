...
(() => {
  ...
  document.addEventListener('change', event => {
    ...
    const error = section.querySelector('[data-db-product-error]');
    if (error) { error.hidden = true; error.textContent = ''; }
    const purchasable = Boolean(variant?.available && !quantityRule.limitReached);
    if (payment) payment.hidden = !purchasable;
    if (button) {
      button.disabled = !purchasable;
      button.textContent = !variant ? 'Unavailable' : !variant.available ? 'Sold out' : quantityRule.limitReached ? 'Maximum quantity in bag' : 'Add to bag';
    }
    if (!variant) return;
    ...
    if (price && prices[String(variant.id)]) price.textContent = prices[String(variant.id)];
  ...
  const validateCartQuantity = input => {
    ...
    const invalid = input.value.trim() === '' || !Number.isInteger(quantity) || quantity < 0 || (quantity !== 0 && (quantity < minimum || quantity % step !== 0 || (maximum !== null && quantity > maximum)));
  ...
  document.addEventListener('input', event => {
    ...
    if (event.target.matches('[data-db-quantity]')) {
      event.target.setCustomValidity('');
      const error = event.target.closest('[data-db-product]')?.querySelector('[data-db-product-error]');
      if (error) { error.hidden = true; error.textContent = ''; }
    }
  });
  document.addEventListener('submit', event => {
    ...
    const invalidQuantity = quantityInput?.value.trim() === '' || !Number.isInteger(quantity) || quantity < rule.min || (quantity - rule.min) % rule.step !== 0 || (rule.max !== null && quantity > rule.max);
    if (!variant || !variant.available || rule.limitReached || invalidQuantity) {
      event.preventDefault();
      const message = !variant ? 'Choose an available option.' : !variant.available ? 'This option is sold out.' : rule.limitReached ? 'The maximum quantity is already in your bag.' : `Enter at least ${rule.min} in increments of ${rule.step}${rule.max === null ? '' : `, up to ${rule.max}`}.`;
      const error = section.querySelector('[data-db-product-error]');
      if (error) { error.hidden = false; error.textContent = message; }
      if (invalidQuantity && quantityInput) {
