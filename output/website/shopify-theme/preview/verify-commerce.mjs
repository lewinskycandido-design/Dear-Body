...
verify('Unpriced preview product initially disables purchasing', () => {
  assert.equal(addButton.disabled, true);
  assert.equal(addButton.textContent.trim(), 'Available soon');
  assert.equal(price.textContent.trim(), 'Available soon');
});
