...
with zipfile.ZipFile(root/'DearBody-Vaporwave-Shopify-Theme.zip','w',zipfile.ZIP_DEFLATED) as package:
    for f in files:
...
web_viewer=viewer.replace('href="dearbody/assets/', 'href="/assets/').replace('src="dearbody/assets/', 'src="/assets/').replace('href="DearBody-Vaporwave-Shopify-Theme.zip"','href="/DearBody-Vaporwave-Shopify-Theme.zip"').replace('href="START-HERE.md"','href="/START-HERE.md"').replace('href="DearBody-Vaporwave-Website-Source.zip"','href="/DearBody-Vaporwave-Website-Source.zip"')
(public/'copy-code').mkdir(exist_ok=True)
(public/'copy-code/index.html').write_text(web_viewer)
(public/'DearBody-Vaporwave-Shopify-Theme.zip').write_bytes((root/'DearBody-Vaporwave-Shopify-Theme.zip').read_bytes())
...
print(f'ZIP: {(root/"DearBody-Vaporwave-Shopify-Theme.zip").stat().st_size:,} bytes')
support_files = [
    ...
    'COPY-PASTE-CODE.html', 'THEME-SHA256.json', 'DearBody-Vaporwave-Shopify-Theme.zip',
...
with zipfile.ZipFile(root/'DearBody-Vaporwave-Website-Source.zip', 'w', zipfile.ZIP_DEFLATED) as package:
    for f in files:
        package.write(f, f.relative_to(root).as_posix())
    for name in support_files:
        package.write(root/name, name)
print(f'Full source ZIP: {(root/"DearBody-Vaporwave-Website-Source.zip").stat().st_size:,} bytes')

# The source archive contains only portable source/support files, never node_modules.
(public/'DearBody-Vaporwave-Website-Source.zip').write_bytes((root/'DearBody-Vaporwave-Website-Source.zip').read_bytes())

# Verify exported bytes, portable source contents, and the locally served artifacts.
shopify_zip = root / 'DearBody-Vaporwave-Shopify-Theme.zip'
source_zip = root / 'DearBody-Vaporwave-Website-Source.zip'
with zipfile.ZipFile(shopify_zip) as package:
    assert package.testzip() is None, 'Shopify ZIP integrity check failed'
    assert set(package.namelist()) == set(manifest), 'Unexpected Shopify ZIP paths'
    for relative, digest in manifest.items():
        content = (theme/relative).read_bytes()
        assert package.read(relative) == content, relative
        assert hashlib.sha256(content).hexdigest() == digest, relative
with zipfile.ZipFile(source_zip) as package:
    assert package.testzip() is None, 'Source ZIP integrity check failed'
    for f in files:
        assert package.read(f.relative_to(root).as_posix()) == f.read_bytes(), str(f)
    for name in support_files:
        assert package.read(name) == (root/name).read_bytes(), name
for archive in [shopify_zip, source_zip]:
    assert (public/archive.name).read_bytes() == archive.read_bytes(), archive.name
assert (public/'START-HERE.md').read_bytes() == (root/'START-HERE.md').read_bytes()
assert json.loads(payload) == sources
assert 'assets/db-logo-burgundy.png' in manifest
assert 'assets/db-logo-secondary.svg' in manifest
assert 'assets/dearbody-vaporwave.css' in manifest
assert 'assets/dearbody-bauhaus.css' not in manifest
print(f'Verified both ZIPs: {len(files)} theme files, {len(support_files)} support files, {len(sources)} copy-code entries; source hashes and served downloads match.')
