...
foreach ($orderPath in @((Join-Path $listingRoot 'SHOPIFY_GALLERY_ORDER_2026-09-24.json'), (Join-Path $uploadRoot 'gallery-order.json'))) {
  ...
  $order | Add-Member -NotePropertyName uncapped_revision -NotePropertyValue '2026-09-24-v2: 14 upright bottles and centered atomizers; native 1254 x 1254 PNG' -Force
...
foreach ($scent in $review.scents) {
  ...
  $frame.revised = $true
  $frame.reason = 'Regenerated with an upright symmetric glass bottle and centered vertical spray atomizer. Original scent label, packaging colors and setting retained. Native 1254 x 1254 PNG.'
...
$review | Add-Member -NotePropertyName uncapped_revision -NotePropertyValue 'uncapped-corrections-2026-09-24-v2/manifest.json' -Force
...
$gallery = [regex]::Replace($gallery, '<p class="intro">All 168 images.*?</p>', '<p class="intro">The 14 uncapped packaging-reveal images now show upright bottles and centered spray atomizers. These corrected PNGs are 1254 × 1254 pixels. <a href="uncapped-corrections-2026-09-24-v2/review.html">Review all 14 corrected images</a>. Other frames retain their existing assets and review notes.</p>')
$gallery = [regex]::Replace($gallery, '<article class="card" data-source-frame="3".*?</article>', {
  ...
  foreach ($item in $manifest.images) {
    if ($card.Contains($item.source)) {
      ...
      $card = $card.Replace($item.source, $item.source + '?revision=' + $item.sha256.Substring(0,12))
      $card = [regex]::Replace($card, '<span class="badge[^"]*">.*?</span>', '<span class="badge revised">Bottle corrected</span>')
      $card = [regex]::Replace($card, '<details>.*?</details>', '<details><summary>Review notes</summary><p>Regenerated upright bottle with centered vertical spray assembly. Original label, packaging colors and setting retained.</p><a href="uncapped-corrections-2026-09-24-v2/review.html">View corrected collection</a></details>')
      break
    }
  }
  $card
