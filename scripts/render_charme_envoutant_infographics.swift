...
let orange = NSColor(calibratedRed: 244 / 255, green: 88 / 255, blue: 22 / 255, alpha: 1)
...
let working = "\(root)/output/product-listing/charme-envoutant/generated-v01/working"
let final = "\(root)/output/product-listing/charme-envoutant/generated-v01/final"
...
try render(base: profileBase, output: "\(final)/08-product-profile-infographic.png") {
    text("CHARME ENVOÛTANT", x: 110, y: 105, width: 860, height: 44, size: 25, color: oxblood, bold: true, kern: 3)
    text("PRODUCT\nPROFILE", x: 110, y: 175, width: 820, height: 220, size: 78, color: ink, bold: true, lineHeight: 84)
    line(x: 110, y: 438, width: 285, color: orange)
    ...
    text(
        "CLEAR GLASS BOTTLE\nAMBER-ORANGE LIQUID\nGLOSSY BLACK CAP\nVIVID ORANGE CANISTER",
        x: 110, y: 875, width: 815, height: 330, size: 34, color: ink, bold: true, lineHeight: 58
    )

    text("PRODUCT CODE  P10738", x: 110, y: 1695, width: 800, height: 52, size: 29, color: cream, bold: true, kern: 0.5)
    line(x: 110, y: 1775, width: 205, color: orange, height: 6)
}

try render(base: ingredientsBase, output: "\(final)/09-ingredients-and-care-infographic.png") {
    text("CHARME ENVOÛTANT", x: 965, y: 100, width: 940, height: 42, size: 24, color: oxblood, bold: true, kern: 3)
    text("INGREDIENTS", x: 965, y: 170, width: 940, height: 78, size: 61, color: ink, bold: true)
    line(x: 965, y: 274, width: 310, color: orange)
    text("AS PRINTED ON THE PACKAGE", x: 965, y: 320, width: 940, height: 38, size: 20, color: oxblood, bold: true, kern: 1)

    text(
        "Alcohol\nWater (Aqua)\nFragrance (Parfum)\nPEG-40 Hydrogenated Castor Oil\nPropylene Glycol\nCoumarin",
        x: 965, y: 405, width: 430, height: 470, size: 25, color: ink, lineHeight: 53
    )
    text(
        "Limonene\nLinalool\nCinnamal\nEugenol\nBenzyl Benzoate",
        x: 1445, y: 405, width: 420, height: 420, size: 25, color: ink, lineHeight: 53
    )
...
try render(base: closingBase, output: "\(final)/10-closing-hero.png") {
    ...
    text("CHARME ENVOÛTANT", x: 110, y: 220, width: 800, height: 46, size: 27, color: orange, bold: true, kern: 3)
    line(x: 110, y: 292, width: 250, color: orange, height: 6)
}

print("Rendered Charme Envoûtant frames 08-10")
